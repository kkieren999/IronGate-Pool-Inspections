...
async function moveSelectedBooking(event, booking) {
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      const oldRef = doc(db, "availability", booking.preferredDate);
      const newRef = doc(db, "availability", date);
      const oldSnap = await transaction.get(oldRef);
      const newSnap = date === booking.preferredDate ? oldSnap : await transaction.get(newRef);
      if (!newSnap.exists()) throw new Error("The selected new date is not available.");
      const oldData = oldSnap.exists() ? oldSnap.data() : {};
      const newData = newSnap.data() || {};
      const releasedOldSlots = releaseSlot(oldData.slots, booking.preferredTimeSlot, booking.id);
      const reservedNewSlots = reserveSlot(date === booking.preferredDate ? releasedOldSlots : newData.slots, slot, booking);

      if (oldSnap.exists()) {
        transaction.set(oldRef, {
          slots: date === booking.preferredDate ? reservedNewSlots : releasedOldSlots,
          updatedAt: serverTimestamp(),
          updatedBy: ADMIN_EMAIL
        }, { merge: true });
      }
      if (date !== booking.preferredDate) {
...
async function cancelBooking(booking) {
  ...
  try {
    await runTransaction(db, async (transaction) => {
      const bookingRef = doc(db, "bookings", booking.id);
      const availabilityRef = doc(db, "availability", booking.preferredDate);
      const snap = await transaction.get(availabilityRef);
      if (snap.exists()) {
        transaction.set(availabilityRef, {
          slots: releaseSlot(snap.data().slots, booking.preferredTimeSlot, booking.id),
          updatedAt: serverTimestamp(),
          updatedBy: ADMIN_EMAIL
        }, { merge: true });
      }
