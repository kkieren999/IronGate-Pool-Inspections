...
function validateDirectBookingPayload(payload) {
...
async function handleDirectCheckoutSubmit(event) {
  ...
  const validationError = validateDirectBookingPayload(booking);
  if (validationError) {
...
if (form) {
  form.addEventListener("submit", handleDirectCheckoutSubmit, { capture: true });
}
